...
In 2019 there were `r cumsum(nineteen$count)` reported bankruptcies in Norway, excluding private persons. 
...
ggplot(rbind(nineteen,twenty), aes(x = factor(month(date)), y = cumsum_region, group=factor(year(date)), color = factor(year(date)))) +
  geom_line() + 
